...
use typenum::{Unsigned, U32, U65};

use crate::native::YubiKeyKemPrivateKey;
...
/// A decap-only version of [`MlKem768P256`] where the private key is stored on a YubiKey.
pub(super) struct YubiKeyMlKem768P256<'a>(PhantomData<&'a ()>);

impl<'a> hpke::Kem for YubiKeyMlKem768P256<'a> {
    type PublicKey = <MlKem768P256 as hpke::Kem>::PublicKey;
    type PrivateKey = YubiKeyKemPrivateKey<'a, MlKem768P256>;

    fn sk_to_pk(_: &Self::PrivateKey) -> Self::PublicKey {
        unreachable!("Never called")
    }

    type EncappedKey = <MlKem768P256 as hpke::Kem>::EncappedKey;
    type NSecret = <MlKem768P256 as hpke::Kem>::NSecret;
    const KEM_ID: u16 = <MlKem768P256 as hpke::Kem>::KEM_ID;
    ...
    fn decap(
        ...
        let (ek_t, dk_pq) = match (sk_recip.recipient(), sk_recip.ml_kem_768_dk()) {
            (crate::recipient::Recipient::MlKem768P256Tag(recipient), Some(dk_pq)) => {
                (recipient.0.ek_t.to_encoded_point(false), dk_pq)
            }
        ...
        let ss_pq = dk_pq
            ...
            .map_err(|()| HpkeError::DecapError)?;
        let ss_t = sk_recip
            .p256_ecdh(encapped_key.ct_t.as_bytes())
            .map_err(|()| HpkeError::DecapError)?;

        let ss = ss(&ss_pq, &ss_t, encapped_key.ct_t.as_bytes(), ek_t.as_bytes());
